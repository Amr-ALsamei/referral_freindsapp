...
import 'package:referral_freindsapp/controller/resultexam_controller.dart';
...
class ResultExamScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    ResultExamController controller=Get.put(ResultExamController());
    return Scaffold(
      appBar: AppBar(title: const Text("نتائج الاختبارات"),),
      body: GetBuilder<ResultExamController>(builder: (controller)=>HandlingDataView(
        statusRequest: controller.statusRequest, 
        widget: Container(
      padding: const EdgeInsets.all(15),
      child:ListView(
        children: [
          GetBuilder<ResultExamController>(builder: (controller)=>Card(
            color: Appcolors.blu,
            child: Column(
