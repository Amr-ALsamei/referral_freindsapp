...
import 'package:referral_freindsapp/Data/Model/questiondaysmodel.dart';
...
late String title ,author ;
  late bool isPlaying;
  final String questionvideo="assets/questionDays";
...
getData()async{
   data.clear();
    ...
    update();
    var response=await questionDaysData.PostData();
    statusRequest=handlingData(response);
       print(response);
    if(StatusRequest.success==statusRequest){
     if(response['status']=="success") {
        List responsebody=response['data'];
        data.addAll(responsebody.map((e) => QuestionDaysModel.fromJson(e)));
        }else{
...
addAnswer(String id,int s)async {
  ...
  if(myServices.sharedPref.getString("usertype")=="children"){
     var response=await questionDaysData.addChildrenAnswerData(
    ...
    var response=await questionDaysData.addAnswerData(
      myServices.sharedPref.getString("Id")!,id,s);
    statusRequest=handlingData(response);
    print(response);
    if(StatusRequest.success==statusRequest){
