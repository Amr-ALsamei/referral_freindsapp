...
class SentenceVideoController extends GetxController{
...
// /////////////////////////////////////////////////
 ...
 checkFavorite()async{
   if(myServices.sharedPref.getString("usertype")=="children"){
    ...
    var response=await sentenceData.addfavoriteData(data);
 ...
 checkWatching()async{
   if(myServices.sharedPref.getString("usertype")=="children"){
    ...
    var response=await sentenceData.addWatchingData(data);
    statusRequest=handlingData(response);
    print(response);
    if(StatusRequest.success==statusRequest){
     if(response['status']=="success") {
 ...
 @override
  void onInit() {
    ...
    sentenceid=Get.arguments["sentenceId"];
    isMute = false;
