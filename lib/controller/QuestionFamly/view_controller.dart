...
import 'package:referral_freindsapp/Data/Model/questionfamlymodel.dart';
...
getData()async{
   data.clear();
    ...
    update();
    var response=await questionFamlyData.PostData();
    statusRequest=handlingData(response);
       print(response);
    if(StatusRequest.success==statusRequest){
     if(response['status']=="success") {
        List responsebody=response['data'];
        data.addAll(responsebody.map((e) => QuestionFamlyModel.fromJson(e)));
        }else{
...
addAnswer(String id,int s)async {
  ...
  if(myServices.sharedPref.getString("usertype")=="children"){
     var response=await questionFamlyData.addChildrenAnswerData(
    ...
    var response=await questionFamlyData.addAnswerData(
      myServices.sharedPref.getString("Id")!,id,s);
    statusRequest=handlingData(response);
    print(response);
    if(StatusRequest.success==statusRequest){
